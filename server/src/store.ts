...
function mapTrack(row: QueryResultRow): Track {
  const requirements = Array.isArray(row.requirements) ? row.requirements.map((item: unknown) => String(item)) : [];
  return {
    id: row.id,
    name: row.name,
    ...
    requirements,
    evaluationPolicy: sponsorPolicySchema.parse(row.evaluation_policy ?? {}),
  };
}
...
export async function listHackathons(): Promise<HackathonRecord[]> {
  ...
  const trackRows = await pool.query("select * from tracks where hackathon_id = any($1::text[]) order by created_at asc", [
    hacks.rows.map((row: QueryResultRow) => String(row.id)),
  ]);
  ...
  return hacks.rows.map((row: QueryResultRow) => mapHackathon(row, tracksByHackathon.get(String(row.id)) ?? []));
}
...
export async function markHackathonFunded(params: {
  ...
  try {
    await client.query("begin");
    await client.query(
      "update hackathons set treasury_tx_hash = $2, updated_at = now() where id = $1",
...
export async function listSubmissions(hackathonId: string): Promise<SubmissionRecord[]> {
  ...
  return Promise.all(submissions.rows.map((row: QueryResultRow) => hydrateSubmission(String(row.id))));
}
...
export async function updateAwardProposal(params: {
  ...
  if (params.digest !== undefined) {
    ...
    fields.push(`digest = $${values.length}`);
  }
  if (params.txHash !== undefined) {
    values.push(params.txHash);
    fields.push(`tx_hash = $${values.length}`);
  }
  if (params.reason !== undefined) {
    values.push(params.reason);
    fields.push(`reason = $${values.length}`);
  }
  await pool.query(`update award_proposals set ${fields.join(", ")} where id = $1`, values);
}
...
export async function getApprovalRequestByAwardId(awardId: string): Promise<ApprovalRequest | null> {
  const row = await pool.query("select * from approval_requests where award_id = $1", [awardId]);
  return row.rowCount ? mapApproval(row.rows[0]) : null;
}
...
export async function markApprovalApproved(params: {
  ...
  await pool.query(
    "update approval_requests set signature = $2, status = $3, approved_at = now(), executed_at = case when $3 = 'executed' then now() else executed_at end where award_id = $1",
    [params.awardId, params.signature, params.status],
  );
}
...
export async function getPrizeClaim(id: string): Promise<PrizeClaim | null> {
  const row = await pool.query("select * from prize_claims where id = $1", [id]);
  return mapClaim(row.rows[0]);
}

export async function listPrizeClaims(hackathonId?: string): Promise<PrizeClaim[]> {
  const rows = hackathonId
    ? await pool.query(
    ...
    : await pool.query("select * from prize_claims order by created_at desc");
  return rows.rows.map((row: QueryResultRow) => mapClaim(row)!);
}
...
export async function listJobs(): Promise<Array<{ id: string; type: string; status: string; payload: Record<string, unknown>; lastError: string | null; createdAt: string }>> {
  const rows = await pool.query("select * from jobs order by created_at desc limit 250");
